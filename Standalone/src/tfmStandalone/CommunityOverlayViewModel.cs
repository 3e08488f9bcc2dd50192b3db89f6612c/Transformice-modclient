...
namespace tfmStandalone
{
	public sealed class CommunityOverlayViewModel : BindableBase
	{
		...
		public CommunityOverlayViewModel(GameInfo gameInfo, ClientHook clientHook)
		{
			...
			this.ToggleViewCommand = new DelegateCommand(delegate() {this.IsCollapsed = !this.IsCollapsed;});
			this.WhitelistToggleCommand = new DelegateCommand<CommunityViewModel>(delegate(CommunityViewModel c)
			{
				...
				if (c.IsSelected)
				{
					...
					{
						EnumerableExtensions.ForEach<CommunityViewModel>(this.Blacklist, delegate(CommunityViewModel bc) {bc.IsSelected = false;});
						this.GameInfo.IgnoredCommunities.Clear();
					}
					this.GameInfo.AllowedCommunities.Add(c.Community.ToLowerInvariant());
					this.ClientHook.SendToClient(ConnectionType.Main, new S_GenericChatMessage {Message = string.Format("[{0}] has been added to the whitelist.", c.Community)});
					return;
				}
				this.GameInfo.AllowedCommunities.Remove(c.Community.ToLowerInvariant());
				this.ClientHook.SendToClient(ConnectionType.Main, new S_GenericChatMessage {Message = string.Format("[{0}] has been removed from the whitelist.", c.Community)});
			});
			this.BlacklistToggleCommand = new DelegateCommand<CommunityViewModel>(delegate(CommunityViewModel c)
			{
				...
				if (c.IsSelected)
				{
					...
					{
						EnumerableExtensions.ForEach<CommunityViewModel>(this.Whitelist, delegate(CommunityViewModel bc) {bc.IsSelected = false;});
						this.GameInfo.AllowedCommunities.Clear();
					}
					this.GameInfo.IgnoredCommunities.Add(c.Community.ToLowerInvariant());
					this.ClientHook.SendToClient(ConnectionType.Main, new S_GenericChatMessage {Message = string.Format("[{0}] ignored.", c.Community)});
					return;
				}
				this.GameInfo.IgnoredCommunities.Remove(c.Community.ToLowerInvariant());
				this.ClientHook.SendToClient(ConnectionType.Main, new S_GenericChatMessage {Message = string.Format("[{0}] unignored.", c.Community)});
			});
		}
		public void Initialize()
		{
			...
			{
				list.Add(new CommunityViewModel {Community = keyValuePair.Value.ToUpperInvariant()});
				list2.Add(new CommunityViewModel {Community = keyValuePair.Value.ToUpperInvariant()});
			}
			EnumerableExtensions.ForEach<CommunityViewModel>(from c in list orderby c.Community select c, delegate(CommunityViewModel c) {this.Whitelist.Add(c);});
			EnumerableExtensions.ForEach<CommunityViewModel>(from c in list2 orderby c.Community select c, delegate(CommunityViewModel c) {this.Blacklist.Add(c);});
		}
	}
}
