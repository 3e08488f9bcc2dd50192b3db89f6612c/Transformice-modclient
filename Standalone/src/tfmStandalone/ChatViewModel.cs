...
namespace tfmStandalone
{
	public class ChatViewModel : BindableBase
	{
		...
		{
			ChatViewModel Chat = this;
			this.ChatMessages = new ObservableCollection<ChatMessageViewModel>();
			this.LogCommand = new DelegateCommand(delegate() {chatWindowViewModel.SendCommand("l " + Chat.Name);});
			this.CasierCommand = new DelegateCommand(delegate() {chatWindowViewModel.SendCommand("casier " + Chat.Name);});
			this.JoinCommand = new DelegateCommand(delegate() {chatWindowViewModel.SendCommand("join " + Chat.Name);});
			this.IgnoreCommand = new DelegateCommand(delegate() {chatWindowViewModel.SendClientCommand("tignore " + Chat.Name);});
			this.MumuteCommand = new DelegateCommand(delegate() {chatWindowViewModel.SendCommand("mumute " + Chat.Name);});
			this.KickCommand = new DelegateCommand(delegate() {chatWindowViewModel.SendCommand("kick " + Chat.Name);});
			...
			this.CloseChatCommand = new DelegateCommand(delegate() {chatWindowViewModel.CloseChat(Chat.Name);});
			this.ToggleMuteChatCommand = new DelegateCommand(delegate() {this.IsMuted = !this.IsMuted;});
			this.TogglePinnedCommand = new DelegateCommand(delegate() {chatWindowViewModel.TogglePinChat(Chat.Name);});
		}
		public void AddMessage(string color, string message, bool showNewMessage)
		{
			this.ChatMessages.Add(new ChatMessageViewModel { Color = color, Message = message });
