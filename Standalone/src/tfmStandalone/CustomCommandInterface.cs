	...
	public sealed class CustomCommandInterface
	{
		...
		public void Save()
		{
			...
			array[0] = new XElement("CustomCommands", this.Commands.Select(delegate(KeyValuePair<string, CustomCommand> c)
			{
				...
				select new XElement("Step", new object[] {new XAttribute("Command", s.Command),new XAttribute("Delay", s.Delay)});
		...
		public void Load()
		{
				...
				try
				{
					...
					else
					{
						list = xelement.Descendants("CustomCommand").Select(delegate(XElement e)
						{
							...
							customCommand6.Steps = e.Descendants("Step").Select(delegate(XElement s) {CustomCommandStep customCommandStep = new CustomCommandStep();XAttribute xattribute2 = s.Attribute("Command");customCommandStep.Command = ((xattribute2 != null) ? xattribute2.Value : null);XAttribute xattribute3 = s.Attribute("Delay");customCommandStep.Delay = int.Parse((xattribute3 != null) ? xattribute3.Value : null);return customCommandStep;}).ToList<CustomCommandStep>();
							return customCommand6;
						}).ToList<CustomCommand>();
					}
					foreach (CustomCommand customCommand in list)
					{
						string key = customCommand.Command.Split(new char[] {' '})[0].ToLowerInvariant();
			...
			CustomCommand customCommand2 = new CustomCommand {Command = "badname [name]"};
			customCommand2.Steps.Add(new CustomCommandStep {Command = string.Format("iban [name] 0 {0}", arg),Delay = 2000});
			customCommand2.Steps.Add(new CustomCommandStep {Command = "iban [name] 360 Name violation"});
			this.Commands.Add("badname", customCommand2);
			customCommand2.Validate(ref empty2);
			CustomCommand customCommand3 = new CustomCommand {Command = "color [name] {color}"};
			customCommand3.Steps.Add(new CustomCommandStep {Command = "colornick [name] {color}"});
			customCommand3.Steps.Add(new CustomCommandStep {Command = "colormouse [name] {color}"});
			this.Commands.Add("color", customCommand3);
			customCommand3.Validate(ref empty2);
			CustomCommand customCommand4 = new CustomCommand {Command = "case {name}"};
			customCommand4.Steps.Add(new CustomCommandStep {Command = "profile {name}"});
			customCommand4.Steps.Add(new CustomCommandStep {Command = "casier {name} true"});
			customCommand4.Steps.Add(new CustomCommandStep {Command = "relation {name}"});
			customCommand4.Steps.Add(new CustomCommandStep {Command = "l {name} 1000"});
			this.Commands.Add("case", customCommand4);
			customCommand4.Validate(ref empty2);
			CustomCommand customCommand5 = new CustomCommand {Command = "bhnote {name} <note>"};
			customCommand5.Steps.Add(new CustomCommandStep {Command = "ibanhack {name}",Delay = 2000});
			customCommand5.Steps.Add(new CustomCommandStep {Command = "imute {name} 0 <note>"});
