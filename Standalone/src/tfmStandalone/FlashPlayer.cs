...
{
	public class FlashPlayer : UserControl, IComponentConnector
	{
		...
		public FlashPlayer.ZoomModeEnum ZoomMode
		{
			...
			set
			{
				...
				if (this.WebBrowser.IsLoaded)
				{
					...
					{
						if (zoomMode == FlashPlayer.ZoomModeEnum.Stretch) text = "exactFit";
						else text = "noScale";
					}
					else text = "showAll";
					this.WebBrowser.InvokeScript("SetZoom", new object[] {text});
				}
				if (this.GameSettings != null) this.GameSettings.ZoomMode = this.ZoomMode;
			}
		}
		public FlashPlayer.QualityEnum Quality
		{
			...
			set
			{
				...
				if (this.WebBrowser.IsLoaded)
				{
					...
					{
						if (quality != FlashPlayer.QualityEnum.Medium) text = "high";
						else text = "medium";
					}
					else text = "low";
					this.WebBrowser.InvokeScript("SetQuality", new object[] {text});
				}
				if (this.GameSettings != null) this.GameSettings.Quality = this._quality;
			}
		}
		...
		{
			sbyte[] e = key.Split(new char[] {','}).Select(new Func<string, sbyte>(sbyte.Parse)).ToArray<sbyte>();
		...
		{
			int[] e = vector.Split(new char[] {','}).Select(new Func<string, int>(int.Parse)).ToArray<int>();
		...
		public void InitializeComponent()
		{
			...
			Uri resourceLocator = new Uri("/Transformice;component/controls/flashplayer.xaml", UriKind.Relative);
		...
		void IComponentConnector.Connect(int connectionId, object target)
		{
			if (connectionId == 1)
			{
				((FlashPlayer)target).Loaded += this.FlashPlayerLoaded;
				return;
			}
			if (connectionId != 2)
			{
