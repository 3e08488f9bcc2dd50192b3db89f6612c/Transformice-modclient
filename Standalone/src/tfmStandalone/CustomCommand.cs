...
namespace tfmStandalone
{
	public sealed class CustomCommand
	{
		...
		private void Read()
		{
			...
			string[] array = this.Command.Split(new char[] {' '}, StringSplitOptions.RemoveEmptyEntries);
			this.Alias = array[0];
			...
			if (array.Length > 1)
			{
				for (int i = 1; i < array.Length; i++)
				{
					...
					{
						this.CommandParameters.Add(new CustomCommandParameter {Key = text,Type = CustomCommandParameterType.Multiple});
					}
					else if (text[0] == '{' && text[text.Length - 1] == '}')
					{
						this.CommandParameters.Add(new CustomCommandParameter {Key = text,Type = CustomCommandParameterType.Single});
					}
					else if (text[0] == '<' && text[text.Length - 1] == '>')
					{
						this.CommandParameters.Add(new CustomCommandParameter {Key = text,Type = CustomCommandParameterType.Long});
					}
				}
			}
		}
		public void Execute(string input, Action<string> commandAction)
		{
			...
			if (this.CommandParameters.Count > 0)
			{
				if (this.CommandParameters[0].Type == CustomCommandParameterType.Multiple)
				{
					...
					{
						List<string> list2 = new List<string> {list[i]};
				...
				else if (this.CommandParameters[this.CommandParameters.Count - 1].Type == CustomCommandParameterType.Long)
				{
					...
					for (int k = 0; k < this.CommandParameters.Count - 1; k++) {list3.Add(list[k]);}
