...
namespace tfmStandalone
{
	public sealed class ChatModel
	{
		...
		{
			if (!Directory.Exists(ChatModel.ExePath + "Chat Logs (" + this.GameInfo.Name + ")")) Directory.CreateDirectory(ChatModel.ExePath + "Chat Logs (" + this.GameInfo.Name + ")");
			if (!Directory.Exists(ChatModel.ExePath + "User Notes")) Directory.CreateDirectory(ChatModel.ExePath + "User Notes");
		}
		public void SendWhisper(string name, string message)
		{
			this.ClientHook.SendToServer(ConnectionType.Main, new C_Whisper { Name = name, Message = message });
		}
		public void SendStaffChat(StaffChatType type, string message)
		{
			this.ClientHook.SendToServer(ConnectionType.Main, new C_StaffChatMessage { Type = type, Message = message });
		}
		public void SendTribeChat(string message)
		{
			this.ClientHook.SendToServer(ConnectionType.Main, new C_TribeMessage {Message = message});
		}
		public void SendCommand(string command)
		{
			this.ClientHook.SendToServer(ConnectionType.Main, new C_Command {Command = command});
		}
		...
		{
			if (!File.Exists(string.Concat(new string[] {ChatModel.ExePath, "Chat Logs (", this.GameInfo.Name, ")\\", name, ".txt"})))
			{
				return null;
			}
			string[] array = File.ReadAllLines(string.Concat(new string[] {ChatModel.ExePath, "Chat Logs (", this.GameInfo.Name, ")\\", name, ".txt"}));
			if(array.Length <= messageCount) return array;
		...
		private void WhisperMessageReceived(object sender, S_Whisper sWhisper)
		{
			...
			string str = string.Concat(new string[] {"] [",ServerInfo.TribulleCommunities[sWhisper.Community],"] [",CultureInfo.CurrentCulture.TextInfo.ToTitleCase(sWhisper.SenderName),"] ",sWhisper.Message.Replace("&lt;", "<").Replace("&amp;", "&")});
			...
			WhisperEventArgs e = new WhisperEventArgs {Name = text,Message = message,Community = sWhisper.Community};
			if (flag)
			{
				EventHandler<WhisperEventArgs> whisperSent = this.WhisperSent;
				if (whisperSent != null) whisperSent(this, e);
			}
			else
			{
				EventHandler<WhisperEventArgs> whisperReceived = this.WhisperReceived;
				if (whisperReceived != null) whisperReceived(this, e);
			}
			if (this.GameSettings.LogWhispers) this.LogMessage(text, message2);
		}
		private void StaffChatMessageReceived(object sender, S_StaffChatMessage staffChatMessage)
		{
			...
			StaffChatReceivedEventArgs e = new StaffChatReceivedEventArgs {Type = staffChatMessage.Type,Name = text,Message = message,GeneralChatMessage = staffChatMessage.GeneralChatMessage};
			EventHandler<StaffChatReceivedEventArgs> staffChatReceived = this.StaffChatReceived;
			if (staffChatReceived != null) staffChatReceived(this, e);
			string fileName;
			switch (staffChatMessage.Type)
			{
			case StaffChatType.ArbitreLocal:
			case StaffChatType.ArbitreAll:
				if (!this.GameSettings.LogArbChat) return;
				fileName = "Arbitre";
				break;
			case StaffChatType.ModerationLocal:
			case StaffChatType.ModerationAll:
				if (!this.GameSettings.LogModoChat) return;
				fileName = "Modo";
				break;
			case StaffChatType.Global:
				return;
			case StaffChatType.MapCrew:
				if (!this.GameSettings.LogMapCrewChat) return;
				fileName = "MapCrew";
				break;
			case StaffChatType.LuaTeam:
				if (!this.GameSettings.LogLuaTeamChat) return;
				fileName = "LuaTeam";
				break;
			case StaffChatType.FunCorp:
				if (!this.GameSettings.LogFunCorpChat) return;
				fileName = "FunCorp";
				break;
			case StaffChatType.FashionSquad:
				if (!this.GameSettings.LogFashionSquadChat) return;
		...
		private void TribeMessageReceived(object sender, S_TribeMessage tribeMessage)
		{
			...
			if (tribeChatReceived != null) tribeChatReceived(this, new TribeChatReceivedEventArgs{Message = message});
			if (this.GameSettings.LogTribeChat) this.LogMessage("Tribe", message2);
		}
		private void ServerMsgReceived(object sender, MessageInterceptor.ServerMessageEventArgs e)
		{
			...
			TaskHelpers.UiInvoke(delegate
			{
				...
				if (serverMessageReceived != null) serverMessageReceived(this, new ServerMessageReceivedEventArgs {IsBuffyBan = flag,Message = message});
				if (!flag && this.GameSettings.LogServeurMessages) this.LogMessage("Server", text);
		...
		{
			using (StreamWriter streamWriter = File.AppendText(string.Concat(new string[] {ChatModel.ExePath,"Chat Logs (",this.GameInfo.Name,")\\",fileName,".txt"})))
			{
