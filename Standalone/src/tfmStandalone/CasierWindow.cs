...
{
	public class CasierWindow : PinnableWindow, IComponentConnector
	{
		internal CasierWindow Window;
		internal FlowDocumentScrollViewer FlowDocumentScrollViewer;
		...
		public CasierWindow(string text)
		{
			...
			{
				int num = 0;
				if (list[0].Contains("Currently running sanctions")) num = list.IndexOf(list.Last((string l) => l.EndsWith("\n\n\n")));
				FlowDocument flowDocument = new FlowDocument { FontFamily = new FontFamily("Verdana"), PagePadding = new Thickness(10.0) };
				Paragraph paragraph = new Paragraph { FontSize = 12.0, Margin = new Thickness(0.0), TextAlignment = TextAlignment.Center };
				...
				flowDocument.Blocks.Add(new Paragraph { Margin = new Thickness(0.0), FontSize = 12.0 });
				if (num > 0)
				{
					Paragraph paragraph2 = new Paragraph { FontSize = 12.0,Margin = new Thickness(0.0)};
					paragraph2.Inlines.Add(new Run { Foreground = this.GetColor("#FFC2C2DA"), Text = "Currently running sanctions:" });
					flowDocument.Blocks.Add(paragraph2);
					flowDocument.Blocks.Add(new Paragraph { Margin = new Thickness(0.0), FontSize = 8.0 });
				}
				for (int i = 1; i < list.Count; i++)
				{
					...
					if (casierItem != null)
					{
						if (casierItem.ItemType == CasierItem.Type.Ban || casierItem.ItemType == CasierItem.Type.Mute)
						{
							...
							Paragraph paragraph3 = new Paragraph { Margin = new Thickness(0.0) };
							...
							{
								Paragraph paragraph4 = new Paragraph { Margin = new Thickness(0.0, 2.0, 0.0, 0.0) };
								paragraph4.Inlines.Add(new Run { FontSize = 12.0, Text = "  " });
							...
							Paragraph paragraph5 = new Paragraph { Margin = new Thickness(0.0, 2.0, 0.0, 0.0) };
							paragraph5.Inlines.Add(new Run { FontSize = 12.0, Text = "  " });
						...
						else
						{
							...
							Paragraph paragraph6 = new Paragraph { Margin = new Thickness(0.0) };
							...
							Paragraph paragraph7 = new Paragraph { Margin = new Thickness(0.0, 2.0, 0.0, 0.0) };
							paragraph7.Inlines.Add(new Run { FontSize = 12.0, Text = "  " });
		...
		private SolidColorBrush GetColor(string hexString) => (SolidColorBrush)new BrushConverter().ConvertFrom(hexString);
		private Run CreateRun(string foregroundResourceName, string text, double fontSize = 12.0, bool isBold = false)
		{
			Run run = new Run {Text = text,FontSize = fontSize};
		...
		private string GetTimeString(string givenTime, string startTime, string endTime, string cancellationTime)
		{
			...
			if (!string.IsNullOrWhiteSpace(givenTime)) list.Add(givenTime);
			if (!string.IsNullOrWhiteSpace(startTime) && !string.IsNullOrWhiteSpace(endTime)) list.Add(string.Format("{0} → {1}", startTime, endTime));
			if (!string.IsNullOrWhiteSpace(cancellationTime)) list.Add(cancellationTime);
		...
		private CasierItem ProcessItem(string item, bool isCurrent)
		{
			...
			if (array[0].StartsWith("<b>MUTE")) a = "MUTE";
			else if (array[0].StartsWith("<b>BAN")) a = "BAN";
			else if (array[0].StartsWith("<b>NAME CHANGE")) a = "NAME CHANGE";
			if (a == "MUTE" || a == "BAN")
			{
				...
				{
					input = array[2].Trim();
					if (array[1].Trim() == "<font size='9'>Overriden</font>") sanctionCasierItem.IsOverriden = true;
					else
					{
		...
		public void InitializeComponent()
		{
			...
			Uri resourceLocator = new Uri("/Transformice;component/views/casierwindow.xaml", UriKind.Relative);
		...
		void IComponentConnector.Connect(int connectionId, object target)
		{
			if (connectionId == 1)
			{
				this.Window = (CasierWindow)target;
				return;
			}
			if (connectionId != 2)
			{
			...
			this.FlowDocumentScrollViewer = (FlowDocumentScrollViewer)target;
		}
	}
}
