...
namespace tfmStandalone
{
	public sealed class MenuOverlayViewModel : BindableBase
	{
		...
		public MenuOverlayViewModel(FlashPlayer flashPlayer, GameSettings gameSettings, WindowService windowService)
		{
			...
			{
				this.Qualities.Add(new QualityViewModel(qualityEnum) {IsSelected = (this.GameSettings.Quality == qualityEnum)});
			}
			...
			{
				foreach (ZoomModeViewModel zoomModeViewModel in this.ZoomModes) zoomModeViewModel.IsSelected = false;
				z.IsSelected = true;
			...
			this.OpenSettingsCommand = new DelegateCommand(delegate() {this.WindowService.ShowGameSettingsWindow();});
		}
	}
}
