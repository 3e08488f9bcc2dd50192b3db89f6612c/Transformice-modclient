			...
			try
			{
				if (result.IsCompleted && result.AsyncState is TcpListener)
				{
					...
					if (tcpListenerEx.Active)
					{
						...
						if (this.MainServer != null) this.Stop();
						...
						{ if (this.MainServer == sender) { this.MainServer = null; } }));
						this.MainServer.Start(tcpClient.Client, this.MainServerIP, tcpListenerEx.Port);
						foreach (TcpListenerEx tcpListenerEx2 in this.TcpListeners) tcpListenerEx2.Stop();
					}
				}
			}
			catch (Exception)
