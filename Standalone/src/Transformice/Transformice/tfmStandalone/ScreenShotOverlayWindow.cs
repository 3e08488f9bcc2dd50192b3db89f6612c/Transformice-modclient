...
{
	public class ScreenShotOverlayWindow : Window, IComponentConnector
	{
		...
		public void InitializeComponent()
		{
			...
			Uri resourceLocator = new Uri("/Transformice;component/views/screenshotoverlaywindow.xaml", UriKind.Relative);
		...
		void IComponentConnector.Connect(int connectionId, object target)
		{
			if (connectionId == 1)
			{
				((ScreenShotOverlayWindow)target).PreviewMouseDown += this.OnPreviewMouseDown;
				((ScreenShotOverlayWindow)target).PreviewMouseMove += this.OnPreviewMouseMove;
				return;
			}
			if (connectionId != 2)
			{
			...
			this.CaptureCanvas = (Canvas)target;
		}
	}
}
